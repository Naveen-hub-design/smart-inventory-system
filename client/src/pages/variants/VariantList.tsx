        ...
        <div className="flex flex-col gap-3 mb-5">
          <form onSubmit={handleSearch} className="flex-1 flex gap-2">
            <div className="relative flex-1 group">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by SKU, barcode, product..." className="input-field pl-9" />
            </div>
            <button type="submit" className="btn-primary">Search</button>
          </form>
          <div className="flex flex-wrap gap-2">
            <select value={productFilter} onChange={(e) => { setProductFilter(e.target.value); setPage(1) }} className="input-field w-full sm:w-48">
              <option value="">All Products</option>
              {products.map(p => <option key={p.id} value={p.id}>{p.product_name}</option>)}
            </select>
            <select value={colorFilter} onChange={(e) => { setColorFilter(e.target.value); setPage(1) }} className="input-field w-full sm:w-36">
              <option value="">All Colors</option>
              {uniqueColors.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select value={sizeFilter} onChange={(e) => { setSizeFilter(e.target.value); setPage(1) }} className="input-field w-full sm:w-32">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <th className="table-header text-center">Barcode</th>
                ...
                <tbody>
                    ...
                    <tr key={v.id} className="table-row animate-fade-in" style={{ animationDelay: `${i * 20}ms` }}>
                      <td className="table-cell font-mono text-xs font-medium text-gray-900 dark:text-white">{v.sku}</td>
                      <td className="table-cell">{v.product_name || 'N/A'}</td>
                      <td className="table-cell">{v.size || 'N/A'}</td>
                      <td className="table-cell">
                        <span className="flex items-center gap-2">
                          {v.color && <span className="w-3 h-3 rounded-full border" style={{ backgroundColor: getColorHex(v.color) }} />}
                          {v.color || 'N/A'}
                        </span>
                      </td>
                      ...
                      <td className="table-cell text-right tabular-nums text-gray-500">{v.min_stock}</td>
                      <td className="table-cell text-right tabular-nums">₹{v.cost_price.toLocaleString()}</td>
                      <td className="table-cell text-right font-medium tabular-nums">₹{v.selling_price.toLocaleString()}</td>
                      <td className="table-cell text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => downloadBarcode(v)} className="p-1.5 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors group" title="Download Barcode">
                            <Download className="w-3.5 h-3.5 text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform" />
                          </button>
                          <button onClick={() => printBarcode(v)} className="p-1.5 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors group" title="Print Barcode">
                            <Printer className="w-3.5 h-3.5 text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform" />
                          </button>
                          <button onClick={() => openBarcodeView(v)} className="p-1.5 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors group" title="View Barcode">
                            <Barcode className="w-3.5 h-3.5 text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform" />
                          </button>
                        </div>
                      </td>
                      <td className="table-cell text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => downloadQR(v)} className="p-1.5 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors group" title="Download QR Code">
                            <Download className="w-3.5 h-3.5 text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform" />
                          </button>
                          <button onClick={() => printQR(v)} className="p-1.5 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors group" title="Print QR Code">
                            <Printer className="w-3.5 h-3.5 text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform" />
                          </button>
                          <button onClick={() => openQRView(v)} className="p-1.5 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors group" title="View QR Code">
                            <QrCode className="w-3.5 h-3.5 text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform" />
                          </button>
                        </div>
                      </td>
                      <td className="table-cell text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(v)} className="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Edit">
                            <Edit2 className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                          </button>
                          <button onClick={() => handleDelete(v.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Delete">
        ...
        {qrViewVariant && (
          <div className="space-y-4">
            ...
            <div className="flex justify-center gap-3 pt-2">
              <button onClick={downloadQRFromView} className="btn-primary flex items-center gap-2 text-sm shadow-lg shadow-primary-500/20">
                <Download className="w-4 h-4" /> Download QR
              </button>
              <button onClick={printQRFromView} className="btn-secondary flex items-center gap-2 text-sm">
                <Printer className="w-4 h-4" /> Print QR
              </button>
              <button onClick={closeQRView} className="btn-secondary flex items-center gap-2 text-sm">
      ...
      <Modal open={!!barcodeViewVariant} onClose={closeBarcodeView} title="Barcode" size="sm">
        {barcodeViewVariant && (
          <div className="space-y-4">
            ...
            <div className="flex justify-center gap-3 pt-2">
              <button onClick={downloadBarcodeFromView} className="btn-primary flex items-center gap-2 text-sm shadow-lg shadow-primary-500/20">
                <Download className="w-4 h-4" /> Download Barcode
              </button>
              <button onClick={printBarcodeFromView} className="btn-secondary flex items-center gap-2 text-sm">
                <Printer className="w-4 h-4" /> Print Barcode
              </button>
              <button onClick={closeBarcodeView} className="btn-secondary flex items-center gap-2 text-sm">
                <X className="w-4 h-4" /> Close
              </button>
            </div>
          </div>
