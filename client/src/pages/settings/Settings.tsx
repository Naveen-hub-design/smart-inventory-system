        ...
        <div className="flex-1 min-w-0">
              ...
              <SectionCard title="Profile Information" desc="Update your personal details and avatar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField label="Full Name">
                    <input id="full_name" type="text" className="input-field text-sm py-2" value={profile.full_name} onChange={(e) => updateProfileField('full_name', e.target.value)} placeholder="John Doe" />
                  </FormField>
                  <FormField label="Email">
                    <input id="email" type="email" className="input-field text-sm py-2" value={profile.email} onChange={(e) => updateProfileField('email', e.target.value)} placeholder="john@example.com" />
                  </FormField>
                  <FormField label="Phone">
                    <input id="phone" type="tel" className="input-field text-sm py-2" value={profile.phone} onChange={(e) => updateProfileField('phone', e.target.value)} placeholder="+91 9876543210" />
                  </FormField>
                </div>
                <div className="flex justify-end gap-2 mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                  <button onClick={() => loadAll()} className="btn-secondary text-sm px-4 py-2">Cancel</button>
                  <button onClick={saveProfile} disabled={saving} className="btn-primary text-sm px-4 py-2 flex items-center gap-1.5">
              ...
              <SectionCard title="Change Password" desc="Update your account password">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <FormField label="Current Password">
                    <div className="relative">
                      <input id="current_password" type={showPassword ? 'text' : 'password'} className="input-field text-sm py-2 pr-9" value={passwordForm.current_password} onChange={(e) => setPasswordForm((p) => ({ ...p, current_password: e.target.value }))} />
                  ...
                  <FormField label="New Password">
                    <input id="new_password" type="password" className="input-field text-sm py-2" value={passwordForm.new_password} onChange={(e) => setPasswordForm((p) => ({ ...p, new_password: e.target.value }))} />
                  </FormField>
                  <FormField label="Confirm Password">
                    <input id="confirm_password" type="password" className="input-field text-sm py-2" value={passwordForm.confirm} onChange={(e) => setPasswordForm((p) => ({ ...p, confirm: e.target.value }))} />
                  </FormField>
                </div>
                <div className="flex justify-end mt-3">
                  <button onClick={changePassword} disabled={saving} className="btn-primary text-sm px-4 py-2 flex items-center gap-1.5">
            ...
            <SectionCard title="Company Information" desc="Configure your business details">
              <div className="flex items-center gap-4 mb-5 pb-4 border-b border-gray-100 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {settings.company.company_logo && (
                    <button onClick={handleLogoRemove} className="btn-secondary text-xs px-3 py-1.5 inline-flex items-center gap-1.5 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-900/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField label="Company Name">
                  <input type="text" className="input-field text-sm py-2" value={settings.company.company_name || ''} onChange={(e) => updateSetting('company', 'company_name', e.target.value)} placeholder="My Company" />
                </FormField>
                <FormField label="GST / Tax Number">
                  <input type="text" className="input-field text-sm py-2" value={settings.company.company_gst || ''} onChange={(e) => updateSetting('company', 'company_gst', e.target.value)} placeholder="GSTIN" />
                </FormField>
                <FormField label="Email">
                  <input type="email" className="input-field text-sm py-2" value={settings.company.company_email || ''} onChange={(e) => updateSetting('company', 'company_email', e.target.value)} placeholder="company@example.com" />
                </FormField>
                <FormField label="Phone">
                  <input type="tel" className="input-field text-sm py-2" value={settings.company.company_phone || ''} onChange={(e) => updateSetting('company', 'company_phone', e.target.value)} placeholder="+91 9876543210" />
                </FormField>
                <FormField label="Website">
                  <input type="url" className="input-field text-sm py-2" value={settings.company.company_website || ''} onChange={(e) => updateSetting('company', 'company_website', e.target.value)} placeholder="https://example.com" />
                </FormField>
                <FormField label="Address">
                  <textarea className="input-field text-sm py-2 resize-none" rows={2} value={settings.company.company_address || ''} onChange={(e) => updateSetting('company', 'company_address', e.target.value)} placeholder="123 Business St, City" />
                </FormField>
            ...
            <SectionCard title="Inventory Defaults" desc="Configure inventory management preferences">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField label="Default Low Stock Threshold">
                  <input type="number" min="0" className="input-field text-sm py-2" value={settings.inventory.inventory_low_stock_threshold || '10'} onChange={(e) => updateSetting('inventory', 'inventory_low_stock_threshold', e.target.value)} />
                </FormField>
                <FormField label="Default Product Category">
                  <input type="text" className="input-field text-sm py-2" value={settings.inventory.inventory_default_category || ''} onChange={(e) => updateSetting('inventory', 'inventory_default_category', e.target.value)} placeholder="General" />
                </FormField>
              </div>
            ...
            <SectionCard title="Backup & Restore" desc="Export or import system settings">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                  ...
                  <button onClick={handleExportBackup} className="btn-primary text-sm px-4 py-2 flex items-center gap-1.5 w-fit">
                ...
                <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                  ...
                  <label className="btn-primary text-sm px-4 py-2 flex items-center gap-1.5 w-fit cursor-pointer bg-emerald-600 hover:bg-emerald-700">
          ...
          {activeTab !== 'profile' && activeTab !== 'backup' && activeTab !== 'about' && (
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <button onClick={handleReset} className="btn-secondary text-sm px-4 py-2 flex items-center gap-1.5 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-900/20">
                <RotateCcw className="w-3.5 h-3.5" /> Reset to Defaults
              </button>
              <div className="flex gap-2">
                <button onClick={() => loadAll()} className="btn-secondary text-sm px-4 py-2 flex items-center gap-1.5">
                  <X className="w-3.5 h-3.5" /> Cancel
                </button>
                <button onClick={handleSave} disabled={saving || !isDirty()} className="btn-primary text-sm px-4 py-2 flex items-center gap-1.5">
                  {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
                  Save Changes
                </button>
              </div>
