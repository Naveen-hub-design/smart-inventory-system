    ...
    <div className="space-y-5">
      <div className="card p-4">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <input type="date" value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)} className="input-field w-40 text-sm" />
          <input type="date" value={filterDateTo} onChange={e => setFilterDateTo(e.target.value)} className="input-field w-40 text-sm" />
          ...
          <select value={filterProduct} onChange={e => setFilterProduct(e.target.value)} className="input-field w-48 text-sm">
            <option value="">All Products</option>
            {products.map(p => <option key={p.id} value={p.id}>{p.product_name}</option>)}
          </select>
          <select value={filterPerformance} onChange={e => setFilterPerformance(e.target.value)} className="input-field w-36 text-sm">
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={j} className="table-row">
                        <td className="table-cell py-2 px-3 font-medium text-gray-700 dark:text-gray-300">{metric.label}</td>
                        {compareIds.map(id => {
                          ...
                          return (
                            <td key={id} className={`table-cell py-2 px-3 text-center font-semibold tabular-nums ${isBest ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-900 dark:text-white'}`}>
                  ...
                  <tbody>
                    {data.performance_table.map((r: SupplierPerformanceRow, i: number) => (
                      <tr key={i} className="table-row animate-fade-in" style={{ animationDelay: `${i * 20}ms` }}>
                        <td className="table-cell py-2 px-3 font-medium text-gray-900 dark:text-white">{r.supplier_name}</td>
                        <td className="table-cell py-2 px-3 text-left font-mono text-[11px] text-gray-500">{r.supplier_code}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums">{r.products_supplied}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums">{r.total_orders}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums text-emerald-600">{r.completed_orders}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums text-red-500">{r.cancelled_orders}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums">Rs.{r.total_value.toFixed(0)}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums">Rs.{r.avg_unit_price.toFixed(2)}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums font-mono text-[11px] text-gray-500">{r.avg_product_cost ? `Rs.${r.avg_product_cost.toFixed(2)}` : 'N/A'}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums">{r.on_time_delivery}%</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums font-bold">{r.overall_rating.toFixed(0)}</td>
                  ...
                  <tbody>
                    {data.purchase_recommendations.slice(0, 15).map((pr: PurchaseRecommendation, i: number) => (
                      <tr key={i} className="table-row animate-fade-in" style={{ animationDelay: `${i * 20}ms` }}>
                        ...
                        <td className="table-cell py-2 px-3 text-sm text-emerald-700 dark:text-emerald-400 font-medium">{pr.suggested_supplier}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums">{pr.current_stock}<span className="text-[10px] text-gray-400">/{pr.min_stock}</span></td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums font-semibold text-primary-600">+{pr.suggested_quantity}</td>
                        <td className="table-cell py-2 px-3 text-right tabular-nums">Rs.{pr.estimated_cost.toFixed(0)}</td>
                        <td className="table-cell py-2 px-3 text-center text-xs">{pr.estimated_delivery}</td>
                        <td className="table-cell py-2 px-3 text-center">
                        ...
                        <td className="table-cell py-2 px-3 text-center font-semibold tabular-nums text-primary-600">{pr.confidence}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
