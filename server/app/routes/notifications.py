...
from app.routes.settings import is_notification_enabled
from app import db
...
def generate_alerts():
    ...
    if is_notification_enabled('notify_low_stock'):
    ...
    db.session.commit()
    return jsonify({'message': f'{len(created)} alerts generated', 'count': len(created)}), 200
